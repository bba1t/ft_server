...
# Настройка PHP-FPM для метрик
# pm.status_path = /status - создает endpoint /status для сбора метрик
RUN echo "pm.status_path = /status" >> /etc/php/7.3/fpm/pool.d/www.conf && \
    echo "listen = 9000" >> /etc/php/7.3/fpm/pool.d/www.conf && \
    echo "listen.allowed_clients = 127.0.0.1,172.0.0.0/8" >> /etc/php/7.3/fpm/pool.d/www.conf
